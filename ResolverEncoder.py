#!/usr/bin/env python3
"""Resolver encoder debug GUI (PyQt5).
...
class _DemoSource:
    ...
    def read_timing(self) -> tuple[float, float]:
        # Demo values
        return self._trig_us, self._abz_us
...
class _ScopeWrapper:
    ...
    def connect(self, port: str, elf: str) -> None:
        ...
        self.trig_time = self.scope.get_variable("trig_calc_time_us")
        self.abz_time = self.scope.get_variable("abz_calc_time_us")
        self.demo = False
    ...
    def read_timing(self) -> tuple[float, float]:
        if self.demo or self.scope is None:
            return self.demo_src.read_timing()
        return (
            float(self.trig_time.get_value()),  # type: ignore[call-arg]
            float(self.abz_time.get_value()),   # type: ignore[call-arg]
        )
...
class WaveformWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, scope: _ScopeWrapper) -> None:
        ...
        self.curve_ang = self.plot.plot(pen=pen_ang, name="Angle/pi")
...
class ResolverEncoderDemo(QtWidgets.QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.ref_btn = QtWidgets.QPushButton("Refresh")
        ...
        vbox.addWidget(self.dial, alignment=QtCore.Qt.AlignHCenter)
        ...
        self.lbl_abz.setAlignment(QtCore.Qt.AlignHCenter)
        vbox.addWidget(self.lbl_abz)

        self.lbl_timings = QtWidgets.QLabel("Trig calc: - us    ABZ calc: - us")
        self.lbl_timings.setAlignment(QtCore.Qt.AlignHCenter)
        vbox.addWidget(self.lbl_timings)
    ...
    def _update(self) -> None:
        ...
        self.lbl_angle.setText(
            f"Angle: {deg:.1f} deg  |  Counts/rev: {self._scope.get_counts()}"
        )
        self.lbl_abz.setText(f"A: {a}  B: {b}  Z: {z}")
        now = time.time()
        if now - self._last_timing_update >= self.TIMING_MS / 1000.0:
            trig_us, abz_us = self._scope.read_timing()
            self.lbl_timings.setText(
                f"Trig calc: {trig_us:.3f} us    ABZ calc: {abz_us:.3f} us"
            )
            self._last_timing_update = now
